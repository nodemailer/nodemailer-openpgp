...
class Encrypter extends Transform {
    ...
    _flush(done) {
        ...
        [].concat(this.options.encryptionKeys || []).forEach(pubKey => {
            openpgp.key.readArmored(pubKey.toString()).then(result => {
                pubKeys = pubKeys.concat(result.keys);
            });
        });
        ...
        if (this.options.signingKey) {
            openpgp.key.readArmored(this.options.signingKey.toString()).then(result => {
        ...
        let options = {
            message: openpgp.message.fromText(bodyHeaders + '\r\n\r\n' + body),
            publicKeys: pubKeys,
            armor: true
        };
