    ...
    _flush(done) {
        (async () => {
            try {
                ...
                if (!pubKeys.length) {
                    // sign only
                    headers.push(['Content-Type: multipart/signed; protocol="application/pgp-signature"; micalg=pgp-sha512;'], [' boundary="' + boundary + '"']);
...
module.exports.openpgpEncrypt = function (options) {
    return function (mail, callback) {
        if ((!(options && options.signingKey) || mail.data.shouldSign === false)
